...
const Athlete = (props) => {
    ...
    const photoClass = `flex flex-col gap-4 items-center justify-center w-[340px] max-w-[60vw] bg-hfWhite p-4 flex align-middle justify-center rounded-sm shadow-md ${rotation}`
        ...
        <div className="flex sm:flex-row flex-col gap-14 text-hfDark py-10 items-center justify-center mx-20">
            <div>
                <div className={photoClass}>
                    ...
                    <h1 className='py-1 leading-6 font-bold text-xl text-center'><span className='text-sm opacity-60'>Atleta HF desde {props.since}</span></h1>
                </div>
            </div>
            <div className={`text-hfWhite flex flex-col text-center items-center font-montserrat max-w-[90vw]`}>
                ...
                <h1 className='w-full py-2 leading-6 font-bold text-lg text-right'>- {props.name}</h1>
            </div>
        </div>
    )
}
